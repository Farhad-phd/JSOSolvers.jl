...
@testset "Test iteration limit" begin
  @testset "$name" for (name, solver) in [
    ...
    ("R2N_MINRES", (nlp; kwargs...) -> R2N(nlp, subsolver_type = MinresSolver; kwargs...)),
  ...
  @testset "$(name)-NLS" for (name, fun) in
    [
      ("trunk", trunk),
      ("tron", tron),
      ...
      # (
      #   "R2NLS_QRMumps",
      #   (nlp; kwargs...) -> R2NLS(nlp, subsolver_type = QRMumpsSolver; kwargs...),
      # ),
    ]
