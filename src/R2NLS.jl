...
- `subsolver = QRMumpsSubsolver`: the subproblem solver type or instance. Pass a type (e.g., `QRMumpsSubsolver`, `LSMRSubsolver`, `LSQRSubsolver`, `CGLSSubsolver`) to let the solver instantiate it, or pass a pre-allocated instance of `AbstractR2NLSSubsolver`.
  - Note: the `QRMumpsSubsolver` guards `fill_ratio`, `dense_max_entries`, `max_nvar`, and `max_nnzj` cannot be passed through `R2NLS`. To set them, pass a pre-built instance, e.g. `subsolver = QRMumpsSubsolver(nls; max_nvar = 100_000, max_nnzj = 5_000_000)`. `max_nvar` is compared against `nls.meta.nvar` and `max_nnzj` against `nls.nls_meta.nnzj` (the number of stored Jacobian nonzeros). Both default to `typemax(Int)` (no additional limit). The `fill_ratio` density guard only applies once the dense Jacobian would exceed `dense_max_entries` entries (default `12_000^2`, i.e. a dense `12_000 × 12_000` Jacobian), so dense problems up to that size are still factorized by qr_mumps. The threshold is on `m * n` rather than on `nvar` because the Jacobian is rectangular. If any guard trips, qr_mumps allocation and symbolic analysis are skipped and `R2NLS` returns status `:exception`. These are cheap input-size checks, not bounds on factorization memory or runtime.
